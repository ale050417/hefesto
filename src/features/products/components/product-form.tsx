...
export function ProductForm({
  ...
  // Secciones colapsables (rediseño 2026-07: el form de edición era un scroll
  // interminable). Al EDITAR arranca todo cerrado (se abre lo que se quiere
  // tocar); al CREAR, todo abierto (hay que completar todo).
  const startOpen = mode === "create";
  const [secOpen, setSecOpen] = useState({
    basicos: startOpen,
    precio: startOpen,
    ficha: startOpen,
    publicacion: startOpen,
  });
  const toggleSec = (k: keyof typeof secOpen) =>
    setSecOpen((s) => ({ ...s, [k]: !s[k] }));
  // Si el guardado falla, se abre TODO: un error dentro de una sección
  // cerrada sería invisible. Se llama desde el submit (event handler, no
  // effect: regla react-hooks/set-state-in-effect).
  const openAllSections = () =>
    setSecOpen({ basicos: true, precio: true, ficha: true, publicacion: true });
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    if (!result.ok) {
      // Errores del server (banner + por campo): abrir todo para que ningún
      // error quede escondido en una sección colapsada.
      openAllSections();
  ...
  }, openAllSections); // inválido (RHF): abrir todo para ver los errores
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-4">
      ...
      <EditSection
        title="Datos básicos"
        hint="Nombre, descripción y categoría"
        open={secOpen.basicos}
        onToggle={() => toggleSec("basicos")}
      >
        <div className="field">
      ...
      <EditSection
        title="Precio y tamaños"
        hint="Tamaños, combinaciones, precio por color e insumos"
        open={secOpen.precio}
        onToggle={() => toggleSec("precio")}
      >
        ...
        <div className="field">
          ...
          />
          <div className="text-faint mt-1 text-[11.5px]">
            Costo de los insumos (vaso, argollas, etc.). Se SUMA a cada precio
            que calcules y se descuenta de la ganancia en cada venta. Cargalo
            ANTES de calcular; si lo cambiás, recalculá.
          </div>
        </div>
      </EditSection>

      {/* Ficha técnica de impresión 3D (la tarjeta dorada se reemplazó por la
          sección colapsable: mismo contenido, sin doble marco ni doble título). */}
      <EditSection
        title="Ficha técnica y colores"
        hint="Modo de color, colores, relleno, entrega y dimensiones"
        open={secOpen.ficha}
        onToggle={() => toggleSec("ficha")}
      >
        <div>
          {/* Modo de color */}
      ...
      <EditSection
        title="Publicación"
        hint="Estado, destacado y nuevo"
        open={secOpen.publicacion}
        onToggle={() => toggleSec("publicacion")}
      >
        {mode === "create" ? (
          <div className="field">
