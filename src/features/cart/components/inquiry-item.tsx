...
import {
  type InquiryItem as InquiryItemType,
  useInquiryStore,
} from "@/stores/inquiryStore";

/** Misma fila que `CartItem`, sin edición de precio: acá no se cobra nada. */
export function InquiryItem({ item }: { item: InquiryItemType }) {
  const setQuantity = useInquiryStore((s) => s.setQuantity);
